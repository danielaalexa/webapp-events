...
public class DeleteEventServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        */
         ...
         if (prenotazione.getQuantita() > 1) {
             int quantita = prenotazione.getQuantita() - 1;
             changeQuantita(request, response, request.getSession(), (User) request.getSession().getAttribute("user"),
                     (List<Prenotazione>) request.getSession().getAttribute("prenotazioni"), id, quantita);
         } else {
             prenotazioneRepository.deleteById(id);
